...
import { ShieldCheck, MapPin, Repeat2, Target, ArrowRight, Flame, MessageSquare, ChevronDown, ChevronUp } from "lucide-react";
      ...
      <div className="mx-auto max-w-3xl px-5 md:px-8 py-8 space-y-8">
        ...
        <div className="relative rounded-3xl overflow-hidden aspect-[16/9]">
          ...
          <div className="absolute top-4 left-4 flex gap-2">
            <span className={`inline-flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-full ${isOngoing ? "bg-amber-100 text-amber-800" : "bg-card/90 text-foreground border border-border/60"}`}>
              {isOngoing ? <><Repeat2 className="w-3.5 h-3.5" /> Ongoing</> : <><Target className="w-3.5 h-3.5" /> One-time</>}
            </span>
        ...
        <div>
          ...
          <div className="mt-3 flex flex-wrap items-center gap-4 text-sm text-muted-foreground">
            <div className="flex items-center gap-1.5">
              <img src={campaign.raiser.avatar} alt="" className="w-5 h-5 rounded-full object-cover" />
              <span>{campaign.raiser.name}</span>
            </div>
            <div className="flex items-center gap-1"><MapPin className="w-3.5 h-3.5" /> {campaign.location}</div>
          </div>
          {org && (
            <Link
              to="/org/$id"
        ...
        <div className="bg-card border border-border rounded-3xl p-6">
          {isOngoing ? (
            <div>
              ...
              <p className="text-sm text-muted-foreground mb-4">Choose how you'd like to give. You can change or cancel anytime.</p>
              ...
              >
                {selectedFreq ? `${frequencyLabel[selectedFreq] ?? "Give once"} →` : "Choose a frequency above"}
              </Link>
            ...
            <div>
              ...
              <div className="h-2 rounded-full bg-muted overflow-hidden mb-2">
                <div className="h-full bg-amber-500 rounded-full transition-all" style={{ width: `${pct ?? 0}%` }} />
              </div>
        ...
        <section>
          ...
          <p className="text-muted-foreground leading-relaxed whitespace-pre-line">{campaign.story}</p>
        </section>
            ...
            <div className="grid grid-cols-3 gap-3">
              {campaign.gallery.map((img, i) => (
                <div key={i} className="aspect-square rounded-2xl overflow-hidden">
                  <img src={img} alt="" className="w-full h-full object-cover hover:scale-105 transition duration-300" />
                </div>
              ))}
            </div>
          </section>
        ...
        {campaign.updates.length > 0 && (
          <section>
            <h2 className="font-display text-xl mb-4">Updates <span className="text-muted-foreground text-base font-normal">({campaign.updates.length})</span></h2>
            ...
            {campaign.updates.length > 1 && (
              ...
              >
                {showAllUpdates ? <><ChevronUp className="w-4 h-4" /> Show less</> : <><ChevronDown className="w-4 h-4" /> Show all {campaign.updates.length} updates</>}
              </button>
            )}
          </section>
        )}
        ...
        {campaign.comments.length > 0 && (
          <section>
            <h2 className="font-display text-xl mb-4 flex items-center gap-2">
              ...
              Comments <span className="text-muted-foreground text-base font-normal">({campaign.comments.length})</span>
            </h2>
            <div className="space-y-4">
              {campaign.comments.map((c) => (
                <div key={c.id} className="flex gap-3">
                  <img src={c.avatar} alt="" className="w-8 h-8 rounded-full object-cover shrink-0 mt-0.5" />
                  <div>
